...
datafolderstring = './data/120920_AC_ReprodCheck/';
...
isAliquot = ~logical(1:length(data))';
...
for iRinse = 1:length(data)
    ...
    temp.nameString1 = data(iRinse).fileName(7:7); 
    if strcmp(temp.nameString1, 'G')
        isNotRinse(iRinse) = 1;
    end
    if strcmp(temp.nameString1, 'B')
        isNotRinse(iRinse) = 1;
    end
...
for iRun = 2:2:length(data)
    ...
    % OPZ subtraction
    data(iRun).OPZcorrInt = data(iRun).intensities(20:(end-20),:) - data(iRun-1).intensities(20:(end-20),:);
    ...
    %Flag samples
    temp.sampNameString = data(iRun).fileName(7:7);
    if strcmp(temp.sampNameString, 'G')
        isSamp(iRun) = 1;
    end
    if strcmp(temp.sampNameString, 'B')
        isBlank(iRun) = 1;
    end
...
for iRatio = 2:2:length(data)
    ...
    data(iRatio).Th232238URatio = Th232 ./ U238;
    data(iRatio).U235Th229Ratio = U235 ./ Th229;
...
%     data(iRatio).avg232Th238URatios= mean(data(iRatio).Th232238URatio);
%     data(iRatio).avg235U229ThRatios = mean(data(iRatio).U235Th229Ratio);
end
...
for iStd = 1:length(stdData)
    ...
    stdData(iStd).MassFrCorr232Th238U = stdData(iStd).Th232238URatio .* (mass238/mass232) .^ stdData(iStd).Beta;
%     stdData(iStd).MassFrCorrAvg238U235U = mean(stdData(iStd).MassFrCorr238U235U);
    stdData(iStd).MassFrCorrAvg232Th238U = mean(stdData(iStd).MassFrCorr232Th238U);
...
for iAliquot = 1: length(sampData)

%Flag U and Th aliquots
    temp.sampNameString = sampData(iAliquot).fileName(10:10);
    if strcmp(temp.sampNameString, 'T')
        isAliquot(iAliquot) = 1;
    end
    if strcmp(temp.sampNameString, 'U')
        isAliquot(iAliquot) = 1;
    end
end

aliquotData = sampData(isAliquot);

for iBlank = 1:length(data)
   
    temp.sampNameString = sampData(iAliquot).fileName(7:7);
    if strcmp(temp.sampNameString, 'B')
        isBlank(iBlank) = 1;
    end
...
SC05xBetaAvg = mean([stdData(1:5).BetaAvg]);
SC1xBetaAvg = mean([stdData(1:5).BetaAvg]);
mass229 = 229;



for iMass = 1:2: length(aliquotData)

    aliquotData(iMass).Th232Th229MFC = aliquotData(iMass).Th232Th229Ratio * (mass229 / mass232 ) ^ ...
        stdData((iMass+1)/2).BetaAvg;
    aliquotData(iMass).Avg232Th229ThMFC = mean(aliquotData(iMass).Th232Th229MFC);
    aliquotData(iMass).Unc232Th229ThMFC = (sqrt((mean(std(aliquotData(iMass).Th232Th229MFC)...
        / sqrt(length(aliquotData(iMass).Th232Th229MFC)) ./ aliquotData(iMass).Th232Th229MFC))^2 ...
        + (stdData((iMass+1)/2).U238U235FracUncert^2))) * aliquotData(iMass).Avg232Th229ThMFC;


end

for iMass2 = 2:2:length(aliquotData)

    aliquotData(iMass2).U238U235MFC = aliquotData(iMass2).U238U235Ratio * (mass235 / mass238 ) ^ ...
        stdData(iMass2/2).BetaAvg;
    aliquotData(iMass2).Avg238U235UMFC = mean(aliquotData(iMass2).U238U235MFC);
    aliquotData(iMass2).Unc238U235UMFC = (sqrt((mean(std(aliquotData(iMass2).U238U235MFC)...
        / sqrt(length(aliquotData(iMass2).U238U235MFC)) ./ aliquotData(iMass2).U238U235MFC))^2 ...
        + (stdData(iMass2/2).U238U235FracUncert^2))) * aliquotData(iMass2).Avg238U235UMFC;
...
for iRat = 1:2:length(aliquotData)

    aliquotData(iRat).Avg232Th238U = aliquotData(iRat).Avg232Th229ThMFC / aliquotData(iRat +1).Avg238U235UMFC ...
        / SpikeRat;
    
    aliquotData(iRat).Avg232Th238UUnc = ((SpikeUnc/SpikeRat) + (aliquotData(iRat +1).Unc238U235UMFC ...
        /aliquotData(iRat +1).Avg238U235UMFC) + (aliquotData(iRat).Unc232Th229ThMFC ...
        /aliquotData(iRat).Avg232Th229ThMFC)) * aliquotData(iRat).Avg232Th238U;
    
end

for iMass3 = 1: length(blankData)
    
    blankData(iMass3).noOPZTh232Th229 = blankData(iMass3).intensities(:, 2) ./ blankData(iMass3).intensities(:, 1);

    blankData(iMass3).Th232Th229MFC = blankData(iMass3).noOPZTh232Th229 * (mass229 / mass232 ) ^ ...
        stdData(iMass3+14).BetaAvg;
    blankData(iMass3).Avg232Th229ThMFC = mean(blankData(iMass3).Th232Th229MFC);
    blankData(iMass3).Unc232Th229ThMFC = (sqrt((mean(std(blankData(iMass3).Th232Th229MFC)...
        / sqrt(length(blankData(iMass3).Th232Th229MFC)) ./ blankData(iMass3).Th232Th229MFC))^2 ...
        + (stdData(iMass3+14).U238U235FracUncert^2))) * blankData(iMass3).Avg232Th229ThMFC;

    blankData(iMass3).U238U235MFC = blankData(iMass3).U238U235Ratio * (mass235 / mass238 ) ^ ...
        stdData(iMass3+14).BetaAvg;
    blankData(iMass3).Avg238U235UMFC = mean(blankData(iMass3).U238U235MFC);
    blankData(iMass3).Unc238U235UMFC = (sqrt((mean(std(blankData(iMass3).U238U235MFC)...
        / sqrt(length(blankData(iMass3).U238U235MFC)) ./ blankData(iMass3).U238U235MFC))^2 ...
        + (stdData(iMass3 +14).U238U235FracUncert^2))) * blankData(iMass3).Avg238U235UMFC;
