function SCData = ParseElement2Folder_SCFPB2Function;
...
for iMass = 1:14
    ...
    sampData(2 * iMass).U235Th229Ratios = sampData(2 * iMass - 1).OPZcorrInt(:,3) ./ sampData(2 * iMass).OPZcorrInt(:,1);
    sampData(2 * iMass).avgU235Th229Ratio = mean(sampData(2 * iMass).U235Th229Ratios);
    sampData(2 * iMass).massFracCorr235U229Th = sampData(2 * iMass).U235Th229Ratios * (mass229 / mass235 ) ^ SC05xBetaAvg;
    sampData(2 * iMass).avgMassFracCorr235U229Th = mean(sampData(2 * iMass).massFracCorr235U229Th);
    sampData(2 * iMass).stdError235U229Th = (sqrt((mean(std(sampData(2 * iMass).massFracCorr235U229Th)...
        / sqrt(length(sampData(2 * iMass).massFracCorr235U229Th)) ./ sampData(2 * iMass).massFracCorr235U229Th))^2 ...
        + (stdData(iMass).U238U235FracUncert^2))) * sampData(2 * iMass).avgMassFracCorr235U229Th;
