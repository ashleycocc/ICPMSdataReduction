...
datafolderstring = './data/12142018_AC_UThFPB/';
...
isSC = ~logical(1:length(data))';
isBlank = ~logical(1:length(data))';

for iRun = 2:2:length(data)
    
    % OPZ subtraction
    data(iRun).OPZcorrInt = data(iRun).intensities(30:(end-50),:) - data(iRun-1).intensities(30:(end-50),:);
...
for iRatio = 2:2:length(data)
    ...
    data(iRatio).Th232U238Ratio = Th232 ./U238 ;
    ...
    data(iRatio).avg232Th238URatios = mean(data(iRatio).Th232U238Ratio);
...
%% Apply Betas to samp data

SC05xBetaAvg = mean([stdData(7:14).BetaAvg]);
...
for iMass1x = 16:28
    ...
    sampData(iMass1x).stdError232Th229Th = (sqrt((mean(std(sampData(iMass1x).massFracCorr232Th229Th)...
        ...
        + (stdData(iMass1x-13).U238U235FracUncert^2))) * sampData(iMass1x).avgMassFracCorr232Th229Th;
    ...
    sampData(iMass1x).stdError238U235U = (sqrt((mean(std(sampData(iMass1x).massFracCorr238U235U)...
        ...
        + (stdData(iMass1x-13).U238U235FracUncert^2))) * sampData(iMass1x).avgMassFracCorr238U235U;
...
for iSC = 1:length(SCData)
    
    SCData(iSC).massFracCorr232Th229Th = SCData(iSC).Th232Th229Ratio * (mass229 / mass232 ) ^ SC05xBetaAvg;
    SCData(iSC).avgMassFracCorr232Th229Th = mean(SCData(iSC).massFracCorr232Th229Th);
    SCData(iSC).stdError232Th229Th = (sqrt((mean(std(SCData(iSC).massFracCorr232Th229Th)...
        / sqrt(length(SCData(iSC).massFracCorr232Th229Th)) ./ SCData(iSC).massFracCorr232Th229Th))^2 ...
        + (stdData(iSC).U238U235FracUncert^2))) * SCData(iSC).avgMassFracCorr232Th229Th;
    
    SCData(iSC).massFracCorr238U235U = SCData(iSC).U238U235Ratio * (mass235 / mass238 ) ^ SC05xBetaAvg;
    SCData(iSC).avgMassFracCorr238U235U = mean(SCData(iSC).massFracCorr238U235U);
    SCData(iSC).stdError238U235U = (sqrt((mean(std(SCData(iSC).massFracCorr238U235U)...
        / sqrt(length(SCData(iSC).massFracCorr238U235U)) ./ SCData(iSC).massFracCorr238U235U))^2 ...
        + (stdData(iSC).U238U235FracUncert^2))) * SCData(iSC).avgMassFracCorr238U235U;
    
    SCData(iSC).U235Th229Ratios = SCData(iSC).OPZcorrInt(:,3) ./ SCData(iSC).OPZcorrInt(:,1);
    SC(iSC).avgU235Th229Ratio = mean(SCData(iSC).U235Th229Ratios);
    SCData(iSC).massFracCorr235U229Th = SCData(iSC).U235Th229Ratios * (mass229 / mass235 ) ^ SC05xBetaAvg;
    SCData(iSC).avgMassFracCorr235U229Th = mean(SCData(iSC).massFracCorr235U229Th);
    SCData(iSC).stdError235U229Th = (sqrt((mean(std(SCData(iSC).massFracCorr235U229Th)...
        / sqrt(length(SCData(iSC).massFracCorr235U229Th)) ./ SCData(iSC).massFracCorr235U229Th))^2 ...
        + (stdData(iSC).U238U235FracUncert^2))) * SCData(iSC).avgMassFracCorr235U229Th;

end


%% Apply betas to Blanks

for iBlank = 1:length(BlankData) - 1
    
    BlankData(iBlank).massFracCorr232Th229Th = BlankData(iBlank).Th232Th229Ratio * (mass229 / mass232 ) ^ SC05xBetaAvg;
    BlankData(iBlank).avgMassFracCorr232Th229Th = mean(BlankData(iBlank).massFracCorr232Th229Th);
    BlankData(iBlank).stdError232Th229Th = (sqrt((mean(std(BlankData(iBlank).massFracCorr232Th229Th)...
        / sqrt(length(BlankData(iBlank).massFracCorr232Th229Th)) ./ BlankData(iBlank).massFracCorr232Th229Th))^2 ...
        + (stdData(iBlank).U238U235FracUncert^2))) * BlankData(iBlank).avgMassFracCorr232Th229Th;
    
    BlankData(iBlank).massFracCorr238U235U = BlankData(iBlank).U238U235Ratio * (mass235 / mass238 ) ^ SC05xBetaAvg;
    BlankData(iBlank).avgMassFracCorr238U235U = mean(BlankData(iBlank).massFracCorr238U235U);
    BlankData(iBlank).stdError238U235U = (sqrt((mean(std(BlankData(iBlank).massFracCorr238U235U)...
        / sqrt(length(BlankData(iBlank).massFracCorr238U235U)) ./ BlankData(iBlank).massFracCorr238U235U))^2 ...
        + (stdData(iBlank).U238U235FracUncert^2))) * BlankData(iBlank).avgMassFracCorr238U235U;
...
% pH11 = subplot(5, 2, 10);
% plotHandle11 = plot(sampData(19).massFracCorr238U235U, '.');
...
% pH21 = subplot(5, 2, 10);
% plotHandle21 = plot(sampData(20).massFracCorr232Th229Th, '.');
...
% pH31 = subplot(5, 2, 10);
% plotHandle31 = plot(sampData(20).OPZcorrInt(:, 1), '.');
...
% pH41 = subplot(5, 2, 10);
% plotHandle41 = plot(sampData(19).OPZcorrInt(:, 3), '.');
